...
<div style="text-align:center">
<img src="/images/目标归属.jpg" width="72%">
...
<div style="text-align:center">
<img src="/images/Yolo V1 坐标变换.jpg" width="92%">
...
原始方案中，预测值 $$t_x, ty$$ 和 $$(x, y)$$ 之间，计算方式如下：
...
该方式下，对坐标 $$t_x, t_y$$ 没有限制，因此预测的 `bounding box` 可能出现在任意位置，导致训练不稳定。因此，在 `V2` 内改为预测偏移量，其计算方式如下所示:
...
其中，$$ (c_x, c_y) $$ 表示格点单元相对于图像左上角的坐标；$$p_w, p_h$$ 表示先验框的尺寸 (`bounding box prior`)，预测值为 $$ t_x, t_y, t_w, t_h, t_o $$。
...
<div style="text-align:center">
<img src="/images/Yolo V2 坐标变换.png" width="81%">
<P>图 6 ：Yolo V2 坐标变换</P>
</div><br>
...
<div style="text-align:center">
<img src="/images/DarkNet-19 网络层.png" width="61%">
<P>图 8 ：DarkNet-19 网络层</P>
</div><br>
